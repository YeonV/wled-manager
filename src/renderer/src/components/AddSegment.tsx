...
export default function AddSegment({ devices, handleSegments }: any) {
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          ...
          <Box sx={{ minWidth: '120px' }}>
            ...
            {device && device !== '' && (
              <FormControl fullWidth>
                <InputLabel id="select-device-label">Select a Segment</InputLabel>
                <Select
                  labelId="select-device-label"
                  id="selectDevice"
                  value={segment}
                ...
                >
                  ...
                  {devices
                    .find((dev) => dev.name === device && dev.seg && dev.seg.length && dev.seg)
                    .seg.map((s, i) => (
                      <MenuItem
                        key={i}
                        value={`{"device": "${device}","seg": [${s.start},${s.stop}],"name":"${s.n || 'Segment ' + (i + 1)
                        }"}`}
                      >
